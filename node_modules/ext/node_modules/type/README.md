...
_For validation of more sophisticated input structures (as deeply nested configuration objects) it's
recommended to consider more powerful schema based utlities (as [AJV](https://ajv.js.org/)
...
Aside of general [`ensure`](docs/ensure.md) validation util, following kind of utilities for
recognized JavaScript types are provided:
...
- `errorMessage` - Custom error message. Following placeholders can be used:
  ...
  - `%n` - To be replaced with meaninfgul name (to be passed with `name` option) of validated value.
    Not effective if `name` option is not present
- `name` - Meaningful name for validated value, to be used in error message, assuming it
